...
# Path to the 'soul' binary file
SOUL_BINARY_PATH = "/mnt/data/soul"

def execute_soul(arguments=None):
    """Executes the 'soul' binary with optional arguments."""
    if arguments is None:
        arguments = []
    
    try:
        result = subprocess.run([SOUL_BINARY_PATH] + arguments, capture_output=True, text=True, check=True)
        return result.stdout  # Return the standard output of the binary
    except subprocess.CalledProcessError as e:
        print(f"Error executing 'soul': {e.stderr}")
        return None
from keep_alive import keep_alive
keep_alive()
# insert your Telegram bot token here
bot = telebot.TeleBot('7379990663:AAHbrg7dkuOqF_H-M0LhdeNjrnbIYjJjQOQ')
